...
{
    private $username = ROCKET_CHAT_USERNAME;
    private $password = ROCKET_CHAT_PASSWORD;
    ...
    public function info()
    {
        ...
        if ($response->code == 200 && isset($response->body->success) && $response->body->success == true) {
            ...
            $this->email = $response->body->user->emails[0]->address ? $response->body->user->emails[0]->address : 'denied permission';
    ...
    public function getUserByEmail($email)
    {
        ...
        $users = parent::list_users();

        if ($users) {
            foreach ($users as $user) {
                if ($user->_id !== 'rocket.cat' && $email == $user->emails[0]->address) {
                    return $user->_id;
                    break;
                }
            }
